...
func TestIterator_NewIterator_NilTree_Failure(t *testing.T) {
	...
	performTest := func(t *testing.T, itr dbm.Iterator) { //nolint: thelper
...
func TestIterator_Empty_Invalid(t *testing.T) {
	...
	performTest := func(t *testing.T, itr dbm.Iterator, mirror [][]string) { //nolint: thelper
...
func iteratorSuccessTest(t *testing.T, config *iteratorTestConfig) { //nolint: thelper
	performTest := func(t *testing.T, itr dbm.Iterator, mirror [][]string) { //nolint: thelper
...
func setupIteratorAndMirror(t *testing.T, config *iteratorTestConfig) (dbm.Iterator, [][]string) { //nolint: thelper
	tree := NewMutableTree(memdb.NewMemDB(), 0, false, NewNopLogger())
...
func setupFastIteratorAndMirror(t *testing.T, config *iteratorTestConfig) (dbm.Iterator, [][]string) { //nolint: thelper
	tree := NewMutableTree(memdb.NewMemDB(), 0, false, NewNopLogger())
...
func setupUnsavedFastIterator(t *testing.T, config *iteratorTestConfig) (dbm.Iterator, [][]string) { //nolint: thelper
	tree := NewMutableTree(memdb.NewMemDB(), 0, false, NewNopLogger())
...
func TestNodeIterator_WithEmptyRoot(t *testing.T) {
	itr, err := NewNodeIterator(nil, newNodeDB(memdb.NewMemDB(), 0, DefaultOptions(), NewNopLogger()))
	require.NoError(t, err)
	require.False(t, itr.Valid())

	itr, err = NewNodeIterator([]byte{}, newNodeDB(memdb.NewMemDB(), 0, DefaultOptions(), NewNopLogger()))
	require.NoError(t, err)
	require.False(t, itr.Valid())
}
